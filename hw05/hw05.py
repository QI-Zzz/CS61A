class VendingMachine:
    ...
    def vend(self):
        if self.number == 0:
            ...
            diff = self.price - self.money
            return f'You must add ${diff} more funds.'
        else:
            change = self.money - self.price
            self.number = self.number - self.money // self.price
            self.money = 0
            if change == 0:
    ...
    def add_funds(self,m):
        # money = money + m
        if self.number == 0:
            return f'Inventory empty. Restocking required. Here is your ${m}.'
        self.money += m
...
def store_digits(n):
    ...
    last = Link(n%10, Link.empty)
    return helper(last, n//10)
