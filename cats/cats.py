...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def check_match(para):
        para_word = [word for word in split(lower(remove_punctuation(para)))]
...
def wpm(typed, elapsed):
    ...
    # word_number = len(split(typed))
    return char_number / 5 /elapsed* 60
    # END PROBLEM 4
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    word_diffs = [diff_function(user_word, word, limit) for word in valid_words]
    
    min_diff = min(word_diffs)
...
def shifty_shifts(start, goal, limit):
    ...
    #     count += len(goal)-len(start)
    # elif len(goal) < len(start):
    #     count += len(start)-len(goal)
    
    # return count
    i = min(len(start), len(goal))
...
def time_per_word(times_per_player, words):
    ...
    for time in times_per_player:
        ...
        for i in range(len(time)-1):
            time_player.append(time[i+1]-time[i])
