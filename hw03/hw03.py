...
def balanced(m):
    ...
    if not balanced(l) or not balanced(r):
        return False
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        l, r = end(left(m)), end(right(m))
        return tree(label, [totals_tree(l), totals_tree(r)])
...
def has_path(t, word):
    ...
    def helper(t, path_so_far):
        new_word = path_so_far + label(t)
        if new_word == word:
            return True
        else:
            for b in branches(t):
                if helper(b, new_word):
                    return True
            return False
...
def check_par():
    ...
    True
    """
    r1 = interval(9, 1) # Replace this line!
...
def quadratic(x, a, b, c):
    ...
    "*** YOUR CODE HERE ***"
    breakpoint = -b/(2*a)
    if upper_bound(x) >= breakpoint >= lower_bound(x):
        if a > 0:
            lower = (4*a*c-b**2)/(4*a)
